...
- **Per-user credential vault** (`packages/backend/src/credential-vault/`): users store their own secrets encrypted at rest (AES-256-GCM via `TokenEncryptionService`, so it needs `TOKEN_ENCRYPTION_KEY`) through `/api/v1/credentials` (create/list/delete, ownership-scoped). The plaintext is write-only — accepted on create, never returned by any read path or logged; the vault fails closed (refuses to store) if no encryption key is configured. Hosting consumes it by **reference, not value**: `host_server` / `deployToCloud` accept `credentialRefs` (env-var name → stored credential name), and `HostingService` resolves them server-side at deploy time via the `CREDENTIAL_RESOLVER` seam (bound to `CredentialVaultService`), merging the decrypted values into the pod env — so a raw secret never transits the agent/tool call. Merge precedence: plaintext `envVars` < vault-resolved `credentialRefs` < platform source-token env (the reserved `MCP_SOURCE_*` keys always win)
- **Marketplace**: Real backend, seeded with 6 servers, frontend Explore uses the real API (not placeholder data)
- **Platform MCP Server**: `POST /mcp` exposes the platform to agents; 10 tools are registered in `packages/backend/src/mcp-server/mcp-tools.service.ts`, including the aggregator pair `search_tools` / `call_tool`, which let one connection discover and invoke the tools of the caller's own hosted servers via `HostedMcpClientService` (`packages/backend/src/hosting/services/hosted-mcp-client.service.ts`). Verified 2026-08-25 with a real `mcpe_` API key: `search_tools` discovered a hosted server's tools and `call_tool` invoked one and returned real data ✅. As of 2026-08-25 the loop is fully agent-drivable through the one connection: `host_server` deploys the server generated in a conversation (same `HostingService.deployToCloud` entry point, same per-tier concurrent-server cap and ownership scoping as the "Host on Cloud" button), and `get_hosted_server` reports the live, cluster-observed readiness so an agent can poll after `host_server` before calling the new tools. These two are built and unit-tested (`mcp-tools.service.spec.ts`, including a real-`McpServer` registration test) and **verified live end-to-end in production 2026-08-25**: over a single `mcpe_`-key MCP connection, an agent drove `generate_mcp_server` (a JSONPlaceholder server — research → tool planning → codegen → hardened Kubernetes sandbox validation → persist, ~3 min) → `host_server` (accepted, `status: deploying`) → `get_hosted_server` polled until the cluster reported `ready` (~57s cold start) → `search_tools` discovered the hosted tools → `call_tool` returned real JSONPlaceholder data. The entire generate→host→poll→discover→call loop ran through the one connection with no human and no context switch. (Cleanup note for that verification: deleting a hosted server must go through `DELETE /api/hosting/servers/:serverId` so the reconciler tears down the k8s Deployment/Service/Secret; deleting only the `hosted_servers` DB row orphans the running workload in the `mcp-servers` namespace.)
...
- **AI**: Single provider-aware seam (`AnthropicService` in `packages/backend/src/ai/anthropic.service.ts`). Each tier resolves to a (provider, model) target: the **default** tier (reasoning/synthesis/codegen, the quality-critical path) runs on Anthropic claude-sonnet-5; the **small** tier (cheap classification/extraction) is routed to **OpenRouter `deepseek/deepseek-v4-flash`** in production (~$0.04/$0.08 per M vs Haiku 4.5's $1/$5). Both tiers default to Anthropic in code — routing is an explicit env decision (`AI_DEFAULT_PROVIDER` / `AI_SMALL_PROVIDER`, `AI_*_MODEL_OPENROUTER`), and the seam fails fast if a tier is routed to a provider whose key is missing. OpenRouter (OpenAI-compatible) support is used for both cost and model breadth; the default/codegen tier is one env var (`AI_DEFAULT_PROVIDER=openrouter`, e.g. `qwen/qwen3-coder`) from an A/B. Verified live 2026-08-25: a real generation's intent step was served by DeepSeek V4 Flash in production (`ai_calls_total{model="deepseek/deepseek-v4-flash"}`, cost $0.000092 for that call, ~18× cheaper than Haiku 4.5 on the same tokens).
...
- **Anthropic API**: claude-sonnet-5 (default/codegen tier) integrated and operational, with token/cost telemetry (Prometheus: `ai_calls_total`, `ai_tokens_total`, `ai_cost_usd_total`; ~$0.22 tracked cost per generation observed)
- **OpenRouter API**: OpenAI-compatible, hundreds of models behind one key; the small tier runs on `deepseek/deepseek-v4-flash` in production (`OPENROUTER_API_KEY` sealed in `mcp-everything-secrets`; routing via `AI_SMALL_PROVIDER=openrouter` in the homelab overlay). Same `ai_*` telemetry, and OpenRouter's real per-call `usage.cost` is recorded when present. Env: `AI_DEFAULT_PROVIDER` / `AI_SMALL_PROVIDER` (`anthropic`|`openrouter`), `AI_DEFAULT_MODEL_OPENROUTER` / `AI_SMALL_MODEL_OPENROUTER`
