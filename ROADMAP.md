...
**2026-08-25 State**: The cloud/Kubernetes deploy path gap above is closed, verified live in production on the homelab k3s cluster: generation (with untrusted code validated in a hardened, throwaway Kubernetes test pod, not local Docker), hosting (a generated server deployed and served MCP over HTTP through the gateway), and the platform's own aggregator MCP server (`search_tools` / `call_tool` discovering and invoking a hosted server's tools through one API-key connection) all ran end-to-end and were independently observed. Several supporting fixes shipped alongside this (Gist publishing now uses the signed-in user's own GitHub account, an SSRF guard on research URLs, Stripe webhook idempotency and an API-version/period bug fix, real `/terms` and `/privacy` pages, a sealed `TAVILY_API_KEY` in the cluster, a dropped unused-Redis health probe). The remaining critical gap is now narrower still: **real payments**. Stripe's code is implemented and correctness-fixed, but no products/prices are configured, so nothing is purchasable yet. A handful of smaller ops/credential gaps also remain (expired `GITHUB_TOKEN`, no email provider for password reset, no database backups configured in this repo, a ~50-60s cold start with no pre-warming/caching for hosted/test pods since each does a from-scratch `npm install` + `tsc`, and the platform runs on a self-hosted homelab cluster rather than a commercial cloud). ArgoCD/image-updater now track `main`, which is the CI-gated branch (lint/typecheck/tests), so the deploy branch and the gated branch are the same.

**2026-08-25 (later that day)**: Two further steps landed. (1) **The full agent loop is now drivable through the single `POST /mcp` connection**: two new MCP tools, `host_server` (deploys the server generated in a conversation via the same `HostingService.deployToCloud` entry point as "Host on Cloud") and `get_hosted_server` (live cluster-observed readiness for polling), close the last gap — hosting was previously REST-only. Verified live end-to-end: an agent ran generate → `host_server` → `get_hosted_server` (polled to ready, ~57s) → `search_tools` → `call_tool` and received real JSONPlaceholder data, all over one `mcpe_` key. (2) **OpenRouter added as a second AI provider behind the seam for cost**: the small tier (classification/extraction) now runs on `deepseek/deepseek-v4-flash` in production (~25-60x cheaper than Haiku 4.5; a measured live intent call cost $0.000092), while the quality-critical codegen tier stays on Claude Sonnet 5 and is one env var from an OpenRouter Qwen3 Coder A/B. Both tiers default to Anthropic in code; routing is explicit config and fails fast if a tier's provider key is missing.
