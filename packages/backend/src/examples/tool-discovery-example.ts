...
export function demonstrateCategoryDistribution() {
  return {
    data: ['extract_user_data', 'parse_config_files', 'aggregate_metrics'],
    api: ['call_rest_endpoint', 'authenticate_user', 'validate_response'],
    analysis: ['analyze_code_quality', 'detect_patterns', 'measure_complexity'],
    utility: ['format_output', 'validate_input', 'generate_ids'],
    build: ['run_tests', 'build_docker_image', 'deploy_service'],
  };
}
